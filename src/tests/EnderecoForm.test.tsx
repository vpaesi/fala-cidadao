...
import * as fetchCidadesModule from '../util/fetchCidades';

jest.mock('../util/fetchCidades', () => ({
  fetchCidades: jest.fn(() => Promise.resolve(['São Paulo', 'Campinas'])),
}));
...
describe('EnderecoForm', () => {
  ...
  it('deve atualizar a cidade ao selecionar um estado', async () => {
    render(<EnderecoForm {...mockProps} />);
    ...
    await waitFor(() => {
      expect(fetchCidadesModule.fetchCidades).toHaveBeenCalledWith('SP');
